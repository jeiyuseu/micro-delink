	...
	<div>
		<Dialog :modal="reloanToggle" :width="'700px'">
					...
					<v-container>
						<v-row>
							<v-col cols="4">
								<v-text-field
									label="SK Cum"
									v-model.number="reloanInfo.skCum"
									readonly
									type="number"
							...
							<v-col cols="4">
								<v-text-field
									label="Past Due"
									v-model.number="reloanInfo.pastDue"
									readonly
									type="number"
									prepend-inner-icon="mdi-currency-php"
									autocomplete="off"
			...
			<div slot="modal-action">
				<v-btn color="orange darken-4" @click="$emit('close-reloan')" text>
					Close
				</v-btn>
				<v-btn color="orange darken-4" class="font-weight-black" @click="reloan" :loading="loading" text>
					Reloan
				</v-btn>
			</div>
		</Dialog>
...
export default {
	...
	computed: {
		...
		fullName: function() {
			return (
				this.reloanInfo.clientInfo.firstName +
				' ' +
				this.reloanInfo.clientInfo.middleInitial +
				' ' +
				this.reloanInfo.clientInfo.lastName
			)
		},
	},
	methods: {
		...mapActions({ GP_RELOAN: 'gp/GP_RELOAN' }),
		reloan: function() {
			if (this.$refs.formReloan.validate()) {
				this.loading = true
				const { uuid, loanAmount } = this.formData
				this.GP_RELOAN({ uuid, loanAmount })
					.then(({ data }) => {
						...
						this.$emit('refresh-reloan-clients')
