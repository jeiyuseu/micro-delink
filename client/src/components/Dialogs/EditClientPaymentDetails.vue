	...
	<div>
		<Dialog :modal="editClientPaymentToggle" :width="'700px'">
			...
			<div slot="modal-text">
				<v-form @submit.prevent="editDetails" ref="formEditClientPaymentDetails">
					<v-row>
						<v-col cols="6">
							<v-text-field
								:value="formatDate(detailsInfo.createdAt)"
						...
						<v-col cols="6">
							<v-text-field :value="formatDate(detailsInfo.updatedAt)" label="Payment Date Updated" disabled>
							</v-text-field
						></v-col>
					</v-row>
					<v-row>
			...
			<div slot="modal-action">
				<v-btn color="primary darken-4" @click="$emit('close-edit-client-payment')" text>
					Close
				</v-btn>
				<v-btn color="primary darken-4" :loading="loading" @click="editDetails" class="font-weight-black" text>
					Update
				</v-btn>
			</div>
		</Dialog>
...
export default {
	...
	methods: {
		...mapActions({
			GP_EDIT_DETAILS: 'gp/GP_EDIT_DETAILS',
		}),
		editDetails() {
			if (this.$refs.formEditClientPaymentDetails.validate()) {
				this.loading = true
				this.GP_EDIT_DETAILS(this.formData)
					.then(({ data }) => {
						this.loading = false
