	...
	<div>
		<card>
						...
						<v-form @submit.prevent="addGp2" ref="formStaffClients">
							<v-container>
								<v-row>
									<v-col cols="12">
										<v-text-field
											v-model.trim="staffClientForm.info.infoDesc"
											label="* New Codename"
				...
				<custom-dialog :modal="dialogUpdate" :width="maxWidth">
					...
					<div slot="modal-text">
						<v-form @submit.prevent="updateGp2" ref="formClientUpdate">
							<v-row>
								<v-col cols="6">
									<v-text-field label="Loan Receivable" class="text-right" :value="this.dialogUpdateInfos.lr.toLocaleString()" readonly>
									</v-text-field>
								</v-col>
								<v-col cols="6">
									<v-text-field label="Weekly Installment" :value="this.dialogUpdateInfos.wi.toLocaleString()" readonly> </v-text-field
								></v-col>
							</v-row>
							<v-row>
								<v-col cols="4">
							...
							<v-card-actions class="justify-end">
								<v-btn
									...
									text
									@click=";(dialogUpdate = !dialogUpdate), resetFields, $refs.formClientUpdate.resetValidation(), resetFields()"
								>
									Close
								</v-btn>
			...
			<div slot="card-text">
				...
				<v-card-title>
					<v-text-field v-model="search" append-icon="mdi-magnify" label="Search code #..." single-line hide-details></v-text-field
				></v-card-title>
				<v-data-table
					:headers="headers"
					:items="filteredData"
...
export default {
	...
	methods: {
			...
			if (this.$refs.formClientUpdate.validate()) {
				...
				this.GP2_UPDATE_CLIENT(this.clientUpdateForm)
					.then(({ data }) => {
						...
						this.resetFields()
						this.filteredData.forEach((value, index1) => {
							if (value.uuid === data.res.uuid) {
								value.gp2Clients = data.res.gp2Clients
								value.totals = data.res.totals
								//check if the client's lr is 0 then remove it from array
								value.gp2Clients = value.gp2Clients.filter((value) => value.lr !== 0)
								//check if clients are empty in gp2Info then remove it from array
								if (value.gp2Clients.length === 0) {
									this.filteredData = this.filteredData.filter((value) => value.uuid !== data.res.uuid)
								}
							}
						})
					})
		...
		dialogUpdateInfo: function(info, item) {
