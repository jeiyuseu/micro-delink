	...
	<div>
		<Card>
			...
			<div slot="card-text">
					...
					<v-card-text>
						<v-row>
							<v-col cols="12">
								...
								<v-btn :to="`${$route.params.codename}/completed-accounts`" outlined rounded color="success" class="mr-3">
								...
								<v-btn @click="exportExcel" :loading="btnExport" :disabled="selected.length === 0" outlined rounded color="success" v-if="!printToggle">
									<v-icon left dark> mdi-check </v-icon>
				...
				<v-text-field solo class="elevation-4 mb-6" v-model="search" append-icon="mdi-magnify" label="Search code #... E.G MA-TA1, MA-TB2, MA-TB3" single-line hide-details></v-text-field>
				<v-data-table
					v-model="selected"
					:headers="headers"
					:items="filteredData"
				...
				>
					<template v-slot:item="{ item, expand, isExpanded, isSelected, select }">
						<tr class="blue darken-4 white--text ">
							<td v-if="!printToggle"><v-simple-checkbox color="white" class="" dark :value="isSelected" @input="select($event)"></v-simple-checkbox></td>
							...
							<td class="text-center">
								...
								<v-tooltip bottom>
									<template v-slot:activator="{ on, attrs }">
										<v-icon class="mr-3" dark v-bind="attrs" @click="dialogDeleteInfo(item)" v-on="on">mdi-delete-circle</v-icon>
									</template>
									<span>Delete</span>
								</v-tooltip>
								<v-tooltip bottom>
					...
					<template v-slot:expanded-item="{ item, headers }">
						...
						<tr v-for="(client, i) in item.gp2Clients" :key="client.uuid">
							...
							<td class="text-center">
								<v-tooltip bottom>
									<template v-slot:activator="{ on, attrs }">
										<v-icon color="success" v-bind="attrs" v-on="on" class="mr-2" @click="dialogEditClient(client, item)">
											mdi-pencil
										</v-icon>
									</template>
		...
		<AddNewCluster :addNewClusterToggle="addNewClusterToggle" @close-new-cluster-toggle="addNewClusterToggle = false" @new-cluster="(data) => filteredData.push(data)" />
		<AddNewClusterClient
		...
		<UpdateClient :updateToggle="updateToggle" :clientInfo="clientInfo" :items="items" @update-clients="refreshUpdateClients" @close-update-client="updateToggle = false" />
		...
		/>
		<DeleteDialog :dialogDeleteToggle="dialogDeleteToggle" :loading="loading" :deleted="deleted" @close-delete="dialogDeleteToggle = false" />
	</div>
</template>
			...
			exportExcel: async function() {
				...
				try {
					...
					this.selected.forEach((value) => {
						...
						worksheet.getCell(`F${row3}`).value = value.weeksToPay
						...
						value.gp2Clients.forEach((client, index) => {
							...
							worksheet.getCell(`I${row6 + row6_spacing}`).value = value.dateOfFirstPayment ? moment().diff(value.dateOfFirstPayment, 'weeks') : '-'

							row6++
						})
						worksheet.getCell(`C${row8 + row12_spacing_total}`).value = value.totals.lr ? value.totals.lr.toLocaleString() : '-'
						worksheet.getCell(`D${row8 + row12_spacing_total}`).value = value.totals.skCum ? value.totals.skCum.toLocaleString() : '-'
						worksheet.getCell(`E${row8 + row12_spacing_total}`).value = value.totals.pastDue ? value.totals.pastDue.toLocaleString() : '-'
						worksheet.getCell(`G${row8 + row12_spacing_total}`).value = value.totals.wi ? value.totals.wi.toLocaleString() : '-'

						row3 += value.gp2Clients.length + 6
						row4 += value.gp2Clients.length + 6
						row5 += value.gp2Clients.length + 6
						row8 += value.gp2Clients.length + 6
						console.log(value.gp2Clients.length)
						row6_spacing += 6
