	...
	<div>
		<card>
									...
									<v-col cols="4">
										<v-text-field
											...
											:rules="[
												...
												(v) => (v.length > 9 ? (v.split('/')[2] >= 1900 && v.split('/')[2] <= 3000) || 'Invalid Year! (Min of 1900 & Max of 3000)' : true),
											]"
											required
										></v-text-field>
									</v-col>
			...
			<div slot="card-text">
				...
				<v-text-field class="elevation-4 mb-6" solo append-icon="mdi-magnify" label="Search clients..." single-line hide-details v-model.trim="search"> </v-text-field>
				<v-data-table
					:headers="headers"
					:items="filteredData"
					:items-per-page="10"
				...
				>
					<template v-slot:item="{ item }">
						<tr>
							<td>
								{{ $titleize(item.firstName + ' ' + item.lastName + ' ' + item.middleInitial) }}
							</td>
							<td>
								{{ item.loanCycle || 0 }}
							</td>
				...
				if (this.$refs.formClient.validate()) {
					...
					this.CLIENT_INSERT_DATA(this.clientForm)
						.then(({ data }) => {
							...
							this.dialog = false
							this.$toasted.success(this.$titleize(this.clientForm.firstName + ' ' + this.clientForm.middleInitial + ' ' + this.clientForm.lastName) + ' is added!', { icon: 'check' })
							this.formReset()
						})
						.catch((error) => {
							this.btnAddClient = false
