...
<script>
	...
	export default {
			...
			addClient: function() {
				if (this.$refs.formClient.validate()) {
					...
					this.CLIENT_INSERT_DATA(this.clientForm)
						...
						.catch((error) => {
							this.btnAddClient = false
							this.$toast.error(`${this.clientForm.firstName} ${this.clientForm.middleInitial} ${this.clientForm.lastName} is exist!`.toUpperCase())
						})
				}
			},
		...
		watch: {
			...
			search: function(val) {
				this.$store.commit('clients/CLIENT_SET_SEARCH', val)
				this.loading = true
				if (!this.waitSearch) {
					setTimeout(async () => {
						await this.CLIENT_GET_DATA()
